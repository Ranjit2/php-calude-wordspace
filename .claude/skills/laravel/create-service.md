# Skill: Create Service (Laravel)

## Checklist
- [ ] 1.  Define the single responsibility this service owns
- [ ] 2.  Create interface in `app/Services/Contracts/{Name}Interface.php` if swappable
- [ ] 3.  Create `app/Services/{Name}Service.php` — constructor injection only
- [ ] 4.  Define DTOs for input/output if data crosses layers
- [ ] 5.  Define custom exceptions in `app/Exceptions/`
- [ ] 6.  Bind in `AppServiceProvider::register()` if interface created
- [ ] 7.  Unit test with all dependencies mocked
- [ ] 8.  `./vendor/bin/pint`
...
final class {Name}Service {
    ...
    private function apply({Name} $model, array $data): void {
        $model->update([/* map result to model columns */]);
    }
}
```
